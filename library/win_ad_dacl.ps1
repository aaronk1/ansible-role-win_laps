...
if ($null -ne $domain_server) {
    $common_ad_parameters.Server = $domain_server
}
if ($null -ne $domain_username) {
    $sec_pass = ConvertTo-SecureString -String $domain_password -AsPlainText -Force
    $credential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList $domain_username, $sec_pass
    $common_ad_parameters.Credential = $credential
}
